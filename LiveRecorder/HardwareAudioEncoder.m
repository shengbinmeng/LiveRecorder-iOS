...
- (int) encode:(CMSampleBufferRef)sampleBuffer {
    ...
    if (status == noErr) {
        ...
        NSData *adtsHeader = [self adtsHeaderForPacketLength:rawAAC.length];
        NSMutableData *adtsAAC = [NSMutableData dataWithData:adtsHeader];
        [adtsAAC appendData:rawAAC];
        [self.output didReceiveEncodedAudio:adtsAAC presentationTime:pts];
    } else {
        NSLog(@"AudioConverterFillComplexBuffer failed with status: %d", status);
    }
